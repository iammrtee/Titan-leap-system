...
function injectTwitterPost() {
  ...
  chrome.storage.local.get(['titanleap_payload'], (res) => {
    ...
    const initCheck = setInterval(() => {
      initAttempts++;
      const textBox = document.querySelector('.public-DraftEditor-content');
      if (textBox) {
        ...
        setTimeout(() => {
          if (mediaData && mediaData.length > 0) {
            ...
            // Twitter requires finding the specific file input using the aria-label
            const fileInputs = Array.from(document.querySelectorAll('input[type="file"]'));
            const mediaInput = fileInputs[0];
            ...
            if (mediaInput) {
              ...
              logToScreen("Warning: Could not find media input layer. Trying drag and drop fallback...");
        ...
        clearInterval(initCheck); logToScreen("Failed to locate Tweet box. DOM might have changed.");
      }
    }, 500);
  });
}
        ...
        setTimeout(() => {
          ...
          const checkReady = setInterval(() => {
            ...
            if (publishBtn && publishBtn.getAttribute('aria-disabled') !== 'true') {
              ...
              clearInterval(checkReady); logToScreen("Timeout: Meta processing took over 80 cycles.");
            }
          }, 1000);
        }, 3000);
      } else if (initAttempts > 60) {
...
function injectLinkedInPost() {
  ...
  chrome.storage.local.get(['titanleap_payload'], (res) => {
    ...
    const initCheck = setInterval(() => {
      ...
      if (startPostBtn) {
        clearInterval(initCheck);
        logToScreen("Found 'Start a Post' button. Bypassing...");
        startPostBtn.click();
...
function injectTikTokPost() {
  ...
  chrome.storage.local.get(['titanleap_payload'], (res) => {
    if (!res.titanleap_payload) return;
    const { caption, mediaBase64: mediaData } = res.titanleap_payload;
    if (!mediaData || mediaData.length === 0) { logToScreen("Warning: TikTok explicitly requires Video."); return; }
    ...
    const initCheck = setInterval(() => {
      ...
      // TikTok Studio iframe check
      const iframe = document.querySelector('iframe');
      let dropZone = document.querySelector('.upload-btn-input, .container-upload-btn, .tiktok-upload-btn') || document.body;
      
      if (iframe && iframe.contentDocument) {
          logToScreen("TikTok Studio iframe detected. Searching inside...");
          const iframeInputs = Array.from(iframe.contentDocument.querySelectorAll('input[type="file"]'));
          if (iframeInputs.length > 0) fileInput = iframeInputs[0];
          dropZone = iframe.contentDocument.querySelector('.upload-btn-input, .container-upload-btn') || iframe.contentDocument.body;
      }

      if (fileInput || document.querySelector('div[class*="upload"]')) {
        clearInterval(initCheck);
        logToScreen("TikTok Uploader located.");

        if (fileInput) {
          logToScreen("Direct input injection.");
          fileInput.files = dt.files;
          fileInput.dispatchEvent(new Event('change', { bubbles: true }));
        } else {
          logToScreen("Input missing. Forcing Drag & Drop Sequence.");
        ...
        const captionCheck = setInterval(() => {
          uploadWaitAttempts++;
          let editor = document.querySelector('.public-DraftEditor-content, div[contenteditable="true"]');
          if (iframe && iframe.contentDocument) {
            editor = iframe.contentDocument.querySelector('.public-DraftEditor-content, div[contenteditable="true"]');
          }
          ...
          if (editor && editor.offsetParent !== null) {
            ...
            const checkReady = setInterval(() => {
              ...
              if (iframe && iframe.contentDocument) {
                 buttons = buttons.concat(Array.from(iframe.contentDocument.querySelectorAll('button')));
              }
              ...
              if (postBtn && !postBtn.disabled && !postBtn.className.includes('disabled')) {
                ...
                clearInterval(checkReady); logToScreen("Timed out awaiting Post button.");
              }
            }, 1000);
          } else if (uploadWaitAttempts > 90) {
            clearInterval(captionCheck); logToScreen("TikTok takes too long to verify video. Stopping.");
          }
        }, 1000);
      } else if (initAttempts > 60) {
        clearInterval(initCheck); logToScreen("Failed to find TikTok uploader inside DOM.");
      }
    }, 1000);
  });
}
